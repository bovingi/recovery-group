...
## Python:
Adapted from the Forest Resiliency Data Synthesis Working Group (ANSWER_KEY_ECOSTRESS). Disclaimer: I do not have much experience with Python and used chatGPT for help. I'm not sure if it actually works. If someone wants to flesh this out, feel free to do so!
```{python}
### Load packages
...
### Make paths for data layers
python_subset_poly = gpd.read_file(
  path.join(python_path_subset, '/subset_polys.shp'))
python_subset_points = gpd.read_file(
    path.join(python_path_subset, '/subset_points.shp')) 
python_landfire = path.join(python_path_raster, "LC20_EVT_220.tif")

## Open and clip the data, clipping it to the study area boundary
landfire_crop = rxr.open_rasterio(python_landfire).rio.clip(python_subset_poly.geometry)
...
### rasterize the point geometry into a raster mask
shapes = ((geom, 1) for geom in python_subset_points.geometry)
mask = rasterize(shapes, out_shape=python_landfire.shape, transform=python_landfire.transform)

### extract values at points using the raster mask
values = python_landfire.read(1, masked=TRUE)[mask == 1]
python_subset_points['raster_value'] = values.tolist()

## Save files
### save clipped raster
landfire_crop.rio.to_raster('C:/Users/kjsie/OneDrive/Documents/recovery-group/python_raster_cropped.tif') 
### save extracted point values
python_subset_points.to_file('C:/Users/kjsie/OneDrive/Documents/recovery-group/python_raster_extract.shp')
```
